...
void PWM_CLK_Start(void) 
{
    ...
    PWM_CLK_CLKEN |= PWM_CLK_CLKEN_MASK;
	PWM_CLK_CLKSTBY |= PWM_CLK_CLKSTBY_MASK;
}
...
void PWM_CLK_Stop(void) 
{
    ...
    PWM_CLK_CLKEN &= (uint8)(~PWM_CLK_CLKEN_MASK);
	PWM_CLK_CLKSTBY &= (uint8)(~PWM_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: PWM_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((PWM_CLK_CLKEN & PWM_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = PWM_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = PWM_CLK_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(PWM_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(PWM_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: PWM_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: PWM_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = PWM_CLK_GetSourceRegister();
    uint16 oldDivider = PWM_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = PWM_CLK_CLKEN & PWM_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((PWM_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(PWM_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: PWM_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    PWM_CLK_MOD_SRC |= modeBitMask & (uint8)PWM_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: PWM_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    PWM_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(PWM_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: PWM_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return PWM_CLK_MOD_SRC & (uint8)(PWM_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: PWM_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = PWM_CLK_GetDividerRegister();
    uint8 oldSrc = PWM_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        PWM_CLK_MOD_SRC |= CYCLK_SSS;
        PWM_CLK_MOD_SRC =
            (PWM_CLK_MOD_SRC & (uint8)(~PWM_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: PWM_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return PWM_CLK_MOD_SRC & PWM_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: PWM_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    PWM_CLK_PHASE = clkPhase & PWM_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: PWM_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return PWM_CLK_PHASE & PWM_CLK_PHASE_MASK;
}
